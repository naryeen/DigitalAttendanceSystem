...
def aindex(request):
    ...
    present = Profile.objects.filter(present=True, status='BScIT A').order_by('updated').reverse()
    absent = Profile.objects.filter(present=False, status='BScIT A').order_by('shift')
    ita = True
    context = {
        'ita': ita,
        'scanned': scanned,
...
def it_b_index(request):
    scanned = LastFace.objects.all().order_by('date').reverse()
    present = Profile.objects.filter(present=True, status='BScIT B').order_by('updated').reverse()
    absent = Profile.objects.filter(present=False, status='BScIT B').order_by('shift')
    itb = True
    context = {
        'itb': itb,
        'scanned': scanned,
...
def profiles(request):
    # profiles = Profile.objects.all()
    profiles = Profile.objects.filter(status='BScIT A').order_by('updated').reverse()
...
def bprofiles(request):
    # profiles = Profile.objects.all()
    profiles = Profile.objects.filter(status='BScIT B').order_by('updated').reverse()
...
def download(request):
    ...
    data = []
    for p in present:
        data.append([p.first_name+' '+p.last_name, p.status, 'Present'])
    
    for a in absent:
        data.append([a.first_name+' '+a.last_name, a.status, 'Absent'])

    df = pd.DataFrame(data=data, columns=['Name','status','P/A'])
    df.to_csv('C:\\Users\\Sonam Dendup\\Desktop\\django\\face_recognition_with_Django-master\\report.csv')
